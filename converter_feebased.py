...
def converter_feebased_para_db():
    """Converter DBV Capital_FeeBased.xlsx para CSV e DB mantendo todas as linhas e colunas"""
    
    # Caminhos dos arquivos
    caminho_excel = Path("DBV Capital_FeeBased.xlsx")
    caminho_csv = Path("DBV Capital_FeeBased.csv")
    caminho_db = Path("DBV Capital_FeeBased.db")
    
    print("=== CONVERSÃO DE FEEBASED PARA CSV E DB ===")
    ...
    try:
        ...
        for nome_planilha in planilhas:
            ...
            if len(planilhas) == 1:
                ...
                csv_planilha = Path(f"DBV Capital_FeeBased_{nome_planilha}.csv")
